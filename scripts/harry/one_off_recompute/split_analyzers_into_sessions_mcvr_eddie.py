...
for day in days:
    ...
    sessions = list(rec_samples.query(f'mouse == {mouse} & date == {day}')[['one_name','two_name']].values[0])
    if 'MCVR' in sessions[0]:
        sessions[0] = 'MCVR'
    ...
    python_arg = f"$HOME/.local/bin/uv run --no-sync /exports/eddie/scratch/chalcrow/harry/code/nolanlab-ephys/scripts/harry/split_analyzers_into_sessions_comp.py {mouse} {day}"
